...
void ShowBorrowed(struct  Ledger*  bb, struct   Books* bs ,struct  Clients* ss) {
...
void BorrowBook(struct Ledger* bb, struct Books* bs, struct Clients* ss, int bookid, int userid){
...
void ReturnBook(struct  Ledger* bb, struct Books * bs,struct Clients * cc , int bookid, int userid) {
	for (int i = 0; i <= bb->booksIds.used;  i++)
	{
